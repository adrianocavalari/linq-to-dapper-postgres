...
using Dapper.Contrib.Linq2Dapper.Postgre.Helpers;

namespace Dapper.Contrib.Linq2Dapper.Postgre
{
        ...
        protected override Expression VisitMethodCall(MethodCallExpression node)
        {
            switch (node.Method.Name)
            {
                ...
                case MethodCall.Select:
                    ...
                    var column = ((MemberExpression)((LambdaExpression) ((UnaryExpression) node.Arguments[1]).Operand).Body).Member.Name;
                    //QueryHelper.GetTypeProperties(type);
                    //_writer.SelectType = type;
                    _writer.SelectColumns.Add(column, column);
